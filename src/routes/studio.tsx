...
export const Route = createFileRoute("/studio")({
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "The Studio — Sculpt & Crown Lime Plaster Atelier" },
      {
        property: "og:description",
        content: "Lime, marble dust and full-scale drawing — the method behind every relief.",
      },
...
const METHOD = [
  { n: "I", t: "Listening", d: "A visit or a long call. Light, room, ritual, and the silence a wall is meant to hold." },
  { n: "II", t: "Drawing", d: "Charcoal studies at full scale, pinned to the wall and lived with until the line stops arguing." },
  { n: "III", t: "Pressing", d: "Lime and marble dust worked wet, layer over layer, each pass thinner than the last." },
  { n: "IV", t: "Settling", d: "Six weeks of curing. The surface pales, hardens, and finally takes the light we designed for." },
];
  ...
  return (
    <main className="min-h-screen bg-background text-ink">
        ...
        <div className="mx-auto max-w-6xl">
          ...
          <div className="mt-12 grid gap-10 sm:grid-cols-[1.1fr_1fr]">
            <RevealText className="max-w-lg text-[13px] font-light leading-[2] text-ink-soft">
              Sculpt &amp; Crown began with a single wall in a Lisbon apartment and a bag of lime that refused to
              behave. Ten years on, the method has barely changed: draw it at full scale, press it wet, and let
              time do the finishing. Nothing is cast, nothing is printed, nothing is rushed.
            </RevealText>
            <RevealText delay={0.12} className="max-w-lg text-[13px] font-light leading-[2] text-ink-soft">
              We keep the studio small — three pairs of hands — because a relief carries the rhythm of whoever
              pressed it. Four commissions a year is not scarcity marketing; it is simply how many walls three
        ...
        <div className="mx-auto max-w-6xl">
          ...
          <div className="mt-12 divide-y divide-ink/10 border-y border-ink/10">
            {METHOD.map((s, i) => (
              ...
              >
                <span className="font-display text-sm font-light tracking-[0.3em] text-ink-soft">{s.n}</span>
                <h2 className="font-display text-3xl font-light tracking-[-0.02em]">{s.t}</h2>
                <p className="max-w-xl text-[13px] font-light leading-[1.9] text-ink-soft">{s.d}</p>
              </motion.div>
            ))}
          </div>
          ...
          <dl className="mt-16 grid gap-8 sm:grid-cols-4">
            {FACTS.map((f) => (
              <div key={f.k}>
                <dt className="text-[10px] font-light uppercase tracking-[0.3em] text-ink-soft/60">{f.k}</dt>
                <dd className="mt-3 font-display text-2xl font-light tracking-[-0.01em]">{f.v}</dd>
              </div>
            ))}
          </dl>
        </div>
      ...
      <section className="border-t border-ink/10 px-6 py-[12vh] text-center sm:px-10">
        ...
        />
        <RevealText delay={0.12} className="mx-auto mt-6 max-w-md text-[13px] font-light leading-[1.9] text-ink-soft">
          Studio visits are by appointment, usually on Thursdays when a panel is still open and workable.
        </RevealText>
        <Link
          to="/commissions"
