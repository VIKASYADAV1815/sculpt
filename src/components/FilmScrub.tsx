...
export default function FilmScrub({ src }: { src: string }) {
  ...
  // Responsive transforms without spring for instant 60fps performance
  const width = useTransform(
    scrollYProgress,
    [0, 0.6],
    ["min(56vw, 420px)", "min(92vw, 1280px)"],
  );
  const height = useTransform(scrollYProgress, [0, 0.6], ["38vh", "68vh"]);
  const scale = useTransform(scrollYProgress, [0, 1], [1.05, 1]);
  const capX = useTransform(scrollYProgress, [0, 1], ["4vw", "-28vw"]);
  const bar = useTransform(scrollYProgress, [0, 1], ["0%", "100%"]);
  const grain = useTransform(scrollYProgress, [0, 0.4], [0.35, 0]);

  return (
    <section ref={ref} className="relative h-[160vh] border-t border-ink/10">
      <div className="sticky top-0 flex h-[100dvh] flex-col items-center justify-center overflow-hidden">
        ...
        <motion.p
          style={{ x: capX }}
          className="mt-8 whitespace-nowrap font-display text-[8vw] font-light italic leading-none text-ink/70 will-change-transform sm:text-[3vw]"
        >
